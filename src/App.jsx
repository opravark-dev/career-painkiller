...
export default function App() {
  ...
  const downloadTxt = (text, filename) => {
    trackEvent('download_clicked', { filename });
    ...
    <div style={{ minHeight: '100vh', background: t.bg, color: t.text,
      ...
      <main id="wizard" style={{ maxWidth: 440, margin: '0 auto', padding: '18px 14px 60px' }}>
        ...
        {step === 2 && results && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <ScoreDashboard data={results.data} t={t} />

            <InsightPanel data={results.data} t={t} />

            <ComparisonView
              original={resumeText}
              optimized={results.data?.optimizedResume || ''}
              t={t}
            />

            <DownloadCenter
              t={t}
              onDownloadRequest={(label) => {
                if (!capturedEmail) {
                  setShowLeadCapture(true);
                } else {
                  if (label === 'Optimized Resume') {
                    downloadTxt(results.data?.optimizedResume || '', 'Optimised_Resume.txt');
                  } else if (label === 'Cover Letter') {
                    downloadTxt(results.data?.coverLetter || '', 'Cover_Letter.txt');
                  } else {
                    alert('PDF Report is a Pro feature! Upgrade now.');
                    setShowUpgradeModal(true);
                  }
                }
              }}
      ...
      <LeadCaptureModal
        ...
        onComplete={(email) => {
          ...
          // Trigger the actual download of the optimized resume as the "reward"
          downloadTxt(results.data?.optimizedResume || '', 'Optimised_Resume.txt');
        }}
        score={results?.data?.optimizedScore}
        t={t}
      />
